...
class DiagResult(Base):
    ...
    user_id = Column(String, nullable=False)             # ID utilisateur issu du JWT (claim 'sub')
    ...
    score = Column(Float, nullable=True)                 # Note de santé (0–5)
    ...
    def __repr__(self):
        return (
            f"<DiagResult(id={self.id}, user_id={self.user_id}, image_id={self.image_id}, "
            f"score={self.score}, disease={self.disease})>"
        )
